...
export class GrassGeometry extends THREE.BufferGeometry {
	constructor(size, count, points) {
		...
		for (let i = 0; i < points.length; i++) {
			...
			uvs.push(
				...Array.from({ length: BLADE_VERTEX_COUNT }).flatMap(() => [interpolate(x, surfaceMin, surfaceMax, 0, 1), interpolate(y, surfaceMin, surfaceMax, 0, 1)])
			);
...
class Grass extends THREE.Mesh {
	constructor(size, count, points) {
		const geometry = new GrassGeometry(size, count, points);
